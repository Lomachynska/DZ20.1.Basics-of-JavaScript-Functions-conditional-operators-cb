...
console.log(userObj2.fullName()); // Виведе: "John Smith"
...
function defUpperStr(text) {                                  // функція використовує логічний оператор ||, щоб перевірити, чи передано значення. 
    return (text || 'DEFAULT TEXT').toUpperCase();            // Якщо параметр text не передано, використовується рядок за замовчуванням. Далі результат перетворюється у верхній регістр.
...
function evenFn(n) {
    const result = [];
    for (let i = 1; i <= n; i++) {       // цикл for, щоб перебрати всі числа від 1 до n.  
        if (i % 2 === 0) {               // Якщо число парне (перевіряється за допомогою i % 2 === 0), воно додається до масиву result.
            result.push(i);
        }
    }
    return result;
}                                      // В результаті отримуємо масив з парних чисел, не включаючи 0.
...
function weekFn(n) { // Перевірка на тип даних і дійсний діапазон
    if (typeof n !== 'number' || !Number.isInteger(n) || n < 1 || n > 7) {   // У цій функції спочатку перевіряємо, чи є n дійсним цілим числом у діапазоні від 1 до 7. 
        return null;
    }

    switch (n) {                     // Якщо ні, повертаємо null. Далі, за допомогою оператора switch, повертаємо назву дня тижня, якщо номер в межах дійсного діапазону.
        case 1: 
            return 'Понеділок';
        case 2:
            return 'Вівторок';
        ...
        case 7:
            return 'Неділя';
        default:
            return null; // Цей випадок не повинен виникати
    }
}
...
function ageClassification(n) {   // тернарний оператор // Ця функція використовує тернарні оператори для перевірки діапазонів значень n і повертає відповідні рядки або null в залежності від умов.
    return n < 0 ? null :
           n <= 24 ? 'Дитинство' :
           n <= 44 ? 'Молодість' :
           n <= 65 ? 'Зрілість' :
...
function oddFn(n) {                   
    const result = [];
    let i = 1;                  // починаємо з 1, оскільки 0 не повинен потрапити в масив // // використовуємо змінну i, яка починається з 1 (першого непарного числа)
    
    while (i <= n) {            //  і в циклі while додаємо до масиву result непарні числа, збільшуючи i на 2 в кожній ітерації. Це дозволяє отримати всі непарні числа до n включно.
        result.push(i);
        i += 2;                 // далі до наступного непарного числа
    }
...
function mainFunc(a, b, callback) {
    // Перевірка, чи є callback функцією
    if (typeof callback !== 'function') {
        return false;
    }
    // Виклик функції з параметрами a та b
    return callback(a, b);
}

// Callback функція для генерації випадкового числа
function cbRandom(min, max) {
    return Math.floor(Math.random() * (max - min + 1)) + min;
}

// Callback функція для піднесення до ступеня
function cbPow(num, pow) {
    return Math.pow(num, pow);
}

// Callback функція для додавання
function cbAdd(a, b) {
    return a + b;
}


console.log(mainFunc(2, 5, cbRandom)); // випадкове ціле число в діапазоні 2..5
console.log(mainFunc(2, 5, cbPow));    // 32 (2^5)
console.log(mainFunc(2, 5, cbAdd));    // 7 (2 + 5)
console.log(mainFunc(2, 5, 'not a func')); // false


//  Функція mainFunc:
// Приймає три параметри: a, b і callback.
// Перевіряє, чи є callback функцією. Якщо ні, повертає false.
// Якщо callback — функція, викликає її з параметрами a і b та повертає результат.
// Callback-функції:
// cbRandom(min, max): Генерує випадкове ціле число в діапазоні між min і max включно.
// cbPow(num, pow): Повертає результат піднесення num до ступеня pow за допомогою Math.pow.
// cbAdd(a, b): Повертає суму a і b.
